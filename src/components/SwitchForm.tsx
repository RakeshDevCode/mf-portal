...
import { Client, MutualFund, SwitchRequest } from '../types/transaction';
import { bseApiService } from '../services/bseApi';
import { RefreshCw, ArrowRight, DollarSign, Hash, Wallet } from 'lucide-react';
...
const SwitchForm: React.FC<SwitchFormProps> = ({ 
  ...
  const [formData, setFormData] = useState({
    switchType: 'AMOUNT' as const,
    amount: '',
    units: '',
  });
  const [toScheme, setToScheme] = useState<MutualFund | null>(null);
  ...
  const switchTypeOptions = [
    { value: 'AMOUNT', label: 'By Amount', icon: DollarSign, description: 'Switch specific amount' },
    { value: 'UNITS', label: 'By Units', icon: Hash, description: 'Switch specific units' },
    { value: 'ALL', label: 'All Holdings', icon: Wallet, description: 'Switch all units' },
  ];
  ...
  const validateForm = () => {
    ...
    if (!toScheme) {
      newErrors.toScheme = 'Please select a scheme to switch to';
    }

    if (toScheme && toScheme.schemeCode === selectedScheme.schemeCode) {
      newErrors.toScheme = 'Cannot switch to the same scheme';
    }

    if (formData.switchType === 'AMOUNT') {
    ...
    } else if (formData.switchType === 'UNITS') {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateForm() || !toScheme) return;
    ...
    try {
      const request: SwitchRequest = {
        ...
        amount: formData.switchType === 'AMOUNT' ? parseFloat(formData.amount) : undefined,
        units: formData.switchType === 'UNITS' ? parseFloat(formData.units) : undefined,
      };

      const response = await bseApiService.switchTransaction(request);
  ...
  const calculateEstimatedAmount = () => {
    if (formData.switchType === 'UNITS' && formData.units && selectedScheme.nav) {
  ...
  const calculateEstimatedUnits = () => {
    if (formData.switchType === 'AMOUNT' && formData.amount && selectedScheme.nav) {
      return (parseFloat(formData.amount) / selectedScheme.nav).toFixed(4);
    }
    return '0';
  };

  const calculateToSchemeUnits = () => {
    if (!toScheme) return '0';
    
    let amount = 0;
    if (formData.switchType === 'AMOUNT') {
      amount = parseFloat(formData.amount) || 0;
    } else if (formData.switchType === 'UNITS') {
      amount = (parseFloat(formData.units) || 0) * selectedScheme.nav;
    } else if (formData.switchType === 'ALL' && portfolio) {
      amount = portfolio.currentValue;
    }
    
    return (amount / toScheme.nav).toFixed(4);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg p-6">
      <div className="flex items-center space-x-3 mb-6">
        ...
        <h2 className="text-xl font-semibold text-gray-900">Switch Transaction</h2>
      </div>

      {/* Portfolio Holdings */}
      <div className="p-4 bg-gray-50 rounded-lg mb-6">
        <h3 className="font-medium text-gray-900 mb-4">Current Holdings - {selectedScheme.schemeName}</h3>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <div className="p-4 bg-gray-50 rounded-lg">
              <div className="font-medium text-gray-900">{selectedScheme.schemeName}</div>
              <div className="text-sm text-gray-600">{selectedScheme.amcName}</div>
              <div className="text-sm text-gray-600">NAV: ₹{selectedScheme.nav}</div>
            </div>
          </div>
          ...
          <div>
            <SchemeSelector
              selectedScheme={toScheme}
              onSchemeSelect={setToScheme}
              label="To Scheme"
            />
            {errors.toScheme && (
              <p className="mt-1 text-sm text-red-600">{errors.toScheme}</p>
            )}
          </div>
        </div>
            ...
            {switchTypeOptions.map((option) => {
              ...
              return (
                <button
                  key={option.value}
                  type="button"
                  onClick={() => setFormData(prev => ({ ...prev, switchType: option.value }))}
                  className={`p-4 border rounded-lg transition-colors ${
                    formData.switchType === option.value
          ...
          <div>
            ...
            {formData.amount && (
              <div className="mt-2 text-sm text-blue-600">
                Estimated Units to Switch: {calculateEstimatedUnits()}
              </div>
            )}
          </div>
        )}

        {formData.switchType === 'UNITS' && (
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Switch Units
            </label>
        ...
        {formData.switchType === 'ALL' && (
          <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <h4 className="font-medium text-blue-900 mb-2">Complete Switch</h4>
            <p className="text-sm text-blue-700">
              All {portfolio.units} units will be switched for approximately ₹{portfolio.currentValue.toLocaleString()}
            </p>
          </div>
        )}

        {/* Expected Units in Target Scheme */}
        {toScheme && (
          <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
            <h4 className="font-medium text-green-900 mb-2">Expected Units in {toScheme.schemeName}</h4>
            <p className="text-sm text-green-700">
              Approximately {calculateToSchemeUnits()} units at NAV ₹{toScheme.nav}
            </p>
          </div>
        )}
